...
export default function HRDashboardPage() {
  ...
  const handleReviewSubmit = async () => {
    ...
    if (!targetId) {
      setReviewStatusMsg({ type: 'error', text: 'Unable to identify candidate for review.' });
      return;
    }
    ...
    try {
      ...
      setReviewStatusMsg({
        type: 'success',
        text: `Success! Decision confirmed & real-time ${decision === 'selected' ? 'offer' : 'feedback'} email dispatched to ${candidateEmail}.`
      });
      ...
      setReviewStatusMsg({
        type: 'error',
        text: err?.message || 'Failed to submit review decision. Please check backend connection.'
      });
    } finally {
      setSubmitting(false);
    }
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="pt-2 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  disabled={submitting}
                  className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-semibold text-xs shadow-sm flex items-center gap-2 disabled:opacity-60 cursor-pointer transition-all"
                >
                  {submitting && <RefreshCw className="w-3.5 h-3.5 animate-spin" />}
                  {submitting ? 'Delivering Real-Time Email...' : 'Confirm & Send Real-Time Email'}
                </button>
              </div>
            </div>
          </div>
